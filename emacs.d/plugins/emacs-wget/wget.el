...
;;	$Id: wget.el,v 1.60 2004/02/28 20:26:28 m1378502 Exp $	
...
(defcustom wget-executable-file-extension-list
  '("exe" "sh" "csh" "pl" "rb")
  "*List of file extension that change file permission executable after downloading."
  :group 'wget
  :type '(repeat
	  (restricted-sexp :tag "File extension executable")))
...
When download dir is not directory or unwritable, get error"
  (let ((download wget-download-directory))
    (unless (string= "/" (substring download -1))
      (setq download (concat download "/")))
    ;; Check argument
    (if arg
	(setq download
	      (wget-read-directory-name "Download directory: " download)))
    ;; Check directory
    (catch 'dir
      (while t
	(when (file-exists-p download)
	  (throw 'dir t))
	...
	(setq download
	      (wget-read-directory-name "Download directory: " download))))
    (unless (file-directory-p download)
      (error "%s is not directory." download))
    (unless (file-writable-p download)
      (error "Can't open download directory: %s" download))
    ;; Change Directory
    (save-excursion
      (set-buffer (get-buffer-create wget-process-buffer))
      (cd (expand-file-name download))
      (message "%s" download))))
...
(defun wget-set-file-executable ()
